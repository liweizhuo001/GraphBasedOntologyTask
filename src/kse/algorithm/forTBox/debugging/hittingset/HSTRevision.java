 ...
 * 通过Hitting Set Tree(碰集树)计算最小碰集
 * [2015-5-31]
 * 最小碰集就是unwanted集合，其中包含确定要删除的公理
 * 从unwanted集合中计算refinementSet, 其中包含待添加的公理(relationship)
 * @author Xuefeng Fu
 */
public class HSTRevision {
	String gPath; 
	...
	public void init(){
		...
		for(MIPP mipp : mipps){			
			...
			for(Relationship r: relOfDiag){
				...
				if(r.getEndNode().getProperty(GlobalParams.NAMEPROPERTY).toString().startsWith(GlobalParams.NEGATIVESIGN))
					continue;
	...
	public void goRevising(){				
		try(Transaction tx = graphDB.beginTx()){			
			unWantedSet = this.calculatingHS();	  //在这里就是最小碰集
	 ...
	 * @return
	 */
	public List<Relationship> calculatingHS(){
		System.out.println("In calculating hitting set...");
		List<Relationship> miniHS = new ArrayList<>();
		List<List<Integer>> mipps = transMIPPsToInt();
		//输出mips
		for(List<Integer> mipp: mipps){
			for(Integer rel : mipp){
				System.out.print(rel+"  ");
			}
			System.out.println();
		}
		BuildingHST building = new BuildingHST(mipps);	
		building.run();
		HST root = building.getRoot();
	 ...
	 * @return 整数形式的MIPP集合
	 */
	public List<List<Integer>> transMIPPsToInt(){
		List<List<Integer>> mippsInInt = new ArrayList<List<Integer>>();
		for(MIPP mipp : mipps){			
			List<Relationship> relOfDiag = mipp.getDiagnosis(); //取出诊断
